...
class BetaEstimator:
    mean_title = 'Difference between zero value and mean direction of theoretical distribution, degrees'
    con_title = 'Difference between zero value and concentration of theoretical distribution'
    y_title = 'probability level'
    def __init__(self, arr1, arr2, name1, name2, effect_mu=None, effect_k=None,
    ...
    def show(self, savefile=False,
             mean_title=mean_title, con_title=con_title,
             y_title=y_title):
        if self.is3d:
            f, ax = plt.subplots(figsize=(10,8))
            ax.set_xlabel(mean_title)
            ax.set_ylabel(con_title)
            color = np.zeros((12,3))
            color[:, 0] = np.linspace(0, 1, 12)
            cs = ax.contour(self.effect_mu * 180/np.pi, self.effect_k, self.result,
                           levels = [0.1, 0.2, 0.4, 0.6, 0.8], colors='#969696')
            css = ax.contourf(self.effect_mu * 180/np.pi, self.effect_k, self.result,
                             levels = np.insert(np.linspace(0, 1, 11), 1, 0.05),
                             colors=color)
            ax.clabel(cs)
            f.colorbar(css, label=y_title, ticks=np.insert(np.linspace(0, 1, 11), 1, 0.05))
            ...
            ax[0].set_xlabel(mean_title)
            ax[1].set_xlabel(con_title)
            f.suptitle('{0}\nMWW p-val: {1:.3f}'.format(self.sample1.name, self.MWW_p))
            if savefile:
